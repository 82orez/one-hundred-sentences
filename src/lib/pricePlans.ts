export const pricePlans = [
  // {
  //   id: "testfreecourse2506",
  //   title: "Test - 6월 무료 체험반",
  //   price: 0,
  //   description: [
  //     {
  //       title: "실시간 온라인 무료 특강 진행",
  //       details: ["매주 일요일 오후 8시", "15분 수업", "zoom 사전 설치 필요"],
  //     },
  //     // "기본 학습 어플 제공",
  //     {
  //       title: "수강 기간 내 학습 어플 사용권",
  //       details: ["여행영어 100문장 관련 컨텐츠"],
  //     },
  //   ],
  // },
  {
    id: "cmd0ltb3g0001ftspr9hm2h2c",
    title: "Test - zoom 연결 테스트",
    price: 0,
    description: [
      "zoom 연결 테스트 용도",
      "7/13일(일) 오후 7시부터 15분간",
      // {
      //   title: "주1회(15분) 1:N 온라인 무료 강의",
      //   details: ["매주 일요일 오후 8시 수업", "총 20회 (7월 6일 ~ 11월 23일)", "수업 시작 전, zoom 설치 필요"],
      // },
      // {
      //   title: "온라인 학습 어플 무료 사용권",
      //   details: ["예습, 복습 및 AI 테스트 가능"],
      // },
    ],
  },
  {
    id: "cmce4nkls0001ftfvn4xwicj8",
    title: "대국민 여행영어 뽀개기\n공동체반",
    price: 0,
    introPage: "/course-detail/free",
    description: [
      "박민규(Sean) 대표 강사",
      {
        title: "주1회(15분) 1:N 온라인 무료 강의",
        details: ["매주 일요일 오후 8시 수업", "총 20회 (7월 6일 ~ 11월 23일)", "수업 시작 전, zoom 설치 필요"],
      },
      {
        title: "온라인 학습 어플 무료 사용권",
        details: ["예습, 복습 및 AI 테스트 가능"],
      },
    ],
  },

  //   {
  //     id: "basic",
  //     title: "Basic Plan",
  //     price: 1100,
  //     description: "월 $10 - 주요 기능 포함",
  //   },
  //   {
  //     id: "pro",
  //     title: "Pro Plan",
  //     price: 2200,
  //     description: "월 $30 - 모든 기능 + 추가 옵션!",
  //   },
];
