    ...
    <main className="min-h-screen bg-gradient-to-br from-sky-100 to-white text-gray-800">
      ...
      <section className="mx-auto flex max-w-6xl flex-col items-center gap-0 px-4 py-6 text-center md:flex-row md:gap-6 md:py-12">
        <div className="md:w-1/2">
          ...
          <a
            href="http://pf.kakao.com/_yxkcfn"
            target="_blank"
            className="mt-4 inline-block rounded-xl bg-yellow-300 px-6 py-3 font-semibold text-yellow-900 shadow-lg transition hover:bg-yellow-500 md:mt-8">
            카카오톡 채널 문의
          </a>
        </div>
      ...
      <section className="mx-auto my-6 max-w-6xl px-4 md:my-12">
        ...
        <div className="grid gap-12 md:grid-cols-3 md:gap-10">
          {[
            ["/images/app-intro-1.png", "학습어플 연계 + 실전 연습", "반복 학습과 말하기 훈련을 통해 표현력·응용력·적응력이 동시에 향상!"],
            ...
            <article key={title} className="flex flex-col items-center text-center">
              <div className="relative mb-3 h-60 w-full overflow-hidden rounded-2xl shadow-lg md:mb-6">
                <Image src={src} alt={title} fill className="object-cover" />
              </div>
              <h3 className="mb-2 text-xl font-bold md:mb-3">{title}</h3>
      ...
      <section className="mx-auto flex max-w-4xl flex-col items-center gap-6 px-4 py-8 text-center md:py-12">
        ...
        <p className="max-w-lg text-base">지금 당장 학습 상담을 시작해 보세요. 상담만 받아도 학습 가이드를 드립니다!</p>
